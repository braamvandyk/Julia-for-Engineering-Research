...
2. Plotly / PlotlyJS. Best coice for interactivity. The plot is generated in a browser. Plotly requires an internet connection, while PlotlyJS runs locally.
...
```julia
julia> using Plots # Will use default backend: GR

julia> plotlyjs() # Switch to PlotlyJS - previously installed
Plots.PlotlyJSBackend()

julia> gr() # Switch back to GR backend
Plots.GRBackend()
```
### General use
...
A simple plot is generated with the `plot()` function. An existing plot can be added to with the `plot!()` function. If no plot variable is specified, this will add to the last generated plot. There are also mutating functions to add/modify the axis lables, plot title etc:
...
You can save the plot to file. Both `*.png` and `*.svg` formats are available and selected simply by specifying the extention in the filename.
