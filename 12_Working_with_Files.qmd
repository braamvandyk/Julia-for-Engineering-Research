...
You can do simple file access through base the Julia commands `open`, `read` and `write`:
...
Where `open()` comes in handy, is when you want to manipulate the contents of the file with a function. You can pass a Combining `open()` with a `do` block is the most common way of doing this:
...
#  10
```
Some new things to note in this example: `eachline()` returns an iterator that contains the lines of the file. We can loop through the lines using this and the lines only get read when needed.
...
```julia
parse(Float64, "this is not a value")
# ERROR: ArgumentError: cannot parse "this is not a value" as Float64
```
...
- `delim`: The character used as a delimiter, such as `','`, or `'\t'` (a tab character).
...
One of these packges is `JLD2`. It implements `save` and `load` from `FileIO` for generic Julia variables. `JLD2` replaces the original `JLD` and is often hugely faster. `JLD` is still around, but you probably don't want to use it.

#### Using JLD2
For consistency over many filetypes, we shall look at the `FileIO` interface implemented by JLD2. You can either just install and use `JLD2` or you can install both `FileIO` and `JLD2`, then just use `FileIO`. If you are only going to deal one or two file types, then you may prefer only installing the specific packages, rather than deal with FileIO. Each package will support `load` and `save` functions in addition to their internal functions.

##### JLD2 with `laod` and `save`
The `FileIO` specification requires you to supply a name for each variable you save. This can either by via creating a `Dict`^[A dictionary, or a collection of name and value pairs.], or by passing the names and variables sequentially as parameters:
...
To read the file, you can either read the whole dictionary, or specify the entries you want (using the name you specified):
...
The `CSV` package is one of the fastest (often the fastest, but things can change with new versions of other packages) ways of reading large (VERY large) CSV files in any language^[See https://www.zdnet.com/article/programming-languages-julia-touts-its-speed-edge-over-python-and-r/ as an example. Here Julia and CSV was up to 22x faster than R's `fread` and both R and Julia were faster than Pandas (Python)].

The `CSV` package has a multitude of features. We are only going to look at the most commonly used ones here. You would however spend your time will in reading the full [documentation](https://csv.juliadata.org/stable/index.html) to see other options, like reading data directly from a zip or gzipped file.

We are also going to assume the most common use case, that your data is in a DataFrame object. There is a separate section on DataFrames.
...
- `dataformat`: A date format string for `Date` and `DateTime` values.
...
- `header`: 
    ...
    - If commented or empty rows are present, counting starts at the first non-commented/empty row.
- `normalizenames`:
...
- `skipto`:
    ...
    - Can be used to skip the column names and replace them with specified names in `header`.
...
- `select`:
    ...
    - Can also pass a predicate function
        (i, name) -> keep::Bool
        Only functions for which the function returns `true` are kept.
- `drop`:
...
- `delim`:
    ...
    - Defaults to `','`.
- `ignorerepeated`:
    - If `true`, consequetive delimiters are treated as a single one.
    - Use with caution, as consequetive delimiters can also be used to show a missing value from a column. Some files, however, use fixed column widths and pad with delimiters, such as spaces.
- `quoted`:
...
- `decimal`:
    - Indicate the deciimal character
    - Defaults to `'.'`.
