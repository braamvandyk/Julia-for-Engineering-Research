...
Any commonly used numerical methods you are likely to want to use are already available in a package, generally written by people with PhDs in the relevant field and optimised to the n^th^ degree by researchers who needed this before you did and contributed to the openly available source code. Sometimes, the methods are already built into Julia, as is the case for linear algebra.
