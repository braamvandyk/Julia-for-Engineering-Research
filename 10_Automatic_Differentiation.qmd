...
7
```
ForwardDiff does have the option to handle mutating functions, where the array result is return in the first parameter. Several packages use ForwardDiff as an option integrated in solvers, including Roots.jl and some of the differential equation solvers.
...
`jacobian()` - calculates the Jacobian matrix for a system of equations, as well as hession
