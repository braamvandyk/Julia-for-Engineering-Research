...
The latter is especially important when passing variables to a function you are benchmarking. Since `primes` is a global variable, Julia cannot do full optiisation of the code, as it must be able to handle any changes to `primes` that can cause it's type to change. Benchmarking in the global scope is therefore not best practise. We get around this by passing the interpolated value of the variable, so Julia actually creates a constant value from the variable and passes that. This gives more accurate benchmarking results.
:::
