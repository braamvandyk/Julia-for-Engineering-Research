...
This is a fairly straigforward way of solving a linear system. For dense systems, it is also quite efficient, if your matrix is well-behaved.
...
Similar approaches are used for sparse matrices, including use of LDLt factorisation for indefiite matrices etc. See the manual section on [factorisation](https://docs.julialang.org/en/v1/stdlib/LinearAlgebra/#LinearAlgebra.factorize) for more detail.
...
```julia
julia> using LinearAlgebra # to import factorize()
...
julia> A = factorize(A) # replace A with its factorised form
...
julia> A.U # To access the U part
3×3 Matrix{Float64}:
 2.0  1.0  1.0
 0.0  1.5  0.5
 0.0  0.0  2.66667

julia> A.L # To access the L part
3×3 Matrix{Float64}:
  1.0   0.0       0.0
 -0.5   1.0       0.0
  0.5  -0.333333  1.0

julia> b = [4, 1, 5] # Our first b vector
3-element Vector{Int64}:
 4
 1
 5

julia> A\b # Solve using specialised methods for LU factorisised martrices
3-element Vector{Float64}:
 1.0
 2.0
 1.0

julia> b = [1, 2, 7] # A new b vector
3-element Vector{Int64}:
 1
 2
 7

julia> A\b # Solve without needing to repeat the factorisation
3-element Vector{Float64}:
  1.75
  3.75
 -0.25000000000000006
...
- `dropzero!()` is simlilar to `dropzero()`, but modifies the array directly.
...
julia> A = Float64[1 0 3; # explicitly define as Float64
           -1 1 0;
            2 1 1]
3×3 Matrix{Float64}:
  1.0  0.0  3.0
...
julia> b = Float64[4, 1, 5] # explicitly define as Float64
3-element Vector{Float64}:
 4.0
 1.0
 5.0

julia> prob = LinearProblem(A, b) # Define problem
...
julia> sol = solve(prob) # and solve
u: 3-element Vector{Float64}:
 1.0
 2.0
 1.0
```
Since LinearSolve has to allow for automatic differentiation, it will implicitly use the type of the variables as specified in `A` in `b`. This will result in an error the inputs are arrays of integers and the if the solution contains fractions. To avoid this, we explicitly define the inputs as `Float64`.
...
```julia
 julia> sol = solve(prob, KrylovJL_GMRES())
u: 3-element Vector{Float64}:
 1.0000000000000004
 2.0000000000000004
 0.9999999999999998
...
julia> b1 = Float64[4, 1, 5]
3-element Vector{Float64}:
 4.0
 1.0
 5.0

julia> prob = LinearProblem(A, b1)
LinearProblem. In-place: true
b: 3-element Vector{Float64}:
 4.0
 1.0
...
julia> linsolve = init(prob) # initialize a cache to store the linear system intermediates
...
julia> sol1 = solve(linsolve) # solve the problem, using the cache
u: 3-element Vector{Float64}:
 1.0
 2.0
 1.0

julia> b2 = Float64[1, 2, 7] # another b vector
3-element Vector{Float64}:
 1.0
 2.0
 7.0

julia> linsolve = LinearSolve.set_b(sol1.cache, b2) # update the cache with the new b vector
...
julia> sol2 = solve(linsolve) # and solve again
u: 3-element Vector{Float64}:
  1.75
  3.75
 -0.25000000000000006
